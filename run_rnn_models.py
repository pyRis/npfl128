...
def load_glove(glove_vec_len):
    ...
    hf = h5py.File(("./data/text_representation/embeddingMatrix_" +
                    f"{glove_vec_len}.h5"), "r")
...
def RNN_Model(nodes, vocab_size, embedding_matrix, inputLength,
              glove_vec_len):
    """Define RNN model architecture."""
    model = keras.models.Sequential([
        keras.layers.Embedding(vocab_size, glove_vec_len,  weights=[
                               embedding_matrix], input_length=inputLength),
...
def LSTM_Model(nodes, vocab_size, embedding_matrix, inputLength,
               glove_vec_len):
...
def GRU_Model(nodes, vocab_size, embedding_matrix, inputLength,
              glove_vec_len):
...
def biRNN_Model(nodes, vocab_size, embedding_matrix, inputLength,
                glove_vec_len):
...
def biLSTM_Model(nodes, vocab_size, embedding_matrix, inputLength,
                 glove_vec_len):
...
def biGRU_Model(nodes, vocab_size, embedding_matrix, inputLength,
                glove_vec_len):
...
def run_model(model_name, model_type, category_name, embedding_matrix,
              vocab_size, input_embedding, output_label, nodes, model_epochs,
              inputLength, glove_vec_len, save_model="no"):
    ...
    if(model_string == "SimpleRNN"):
        ...
        model = RNN_Model(nodes, vocab_size, embedding_matrix,
                          inputLength, glove_vec_len)
    elif(model_string == "BiRNN"):
        print("Executing Bidirectional RNN")
        model = biRNN_Model(nodes, vocab_size, embedding_matrix,
                            inputLength, glove_vec_len)
    elif(model_string == "SimpleLSTM"):
        print("Executing Simple LSTM")
        model = LSTM_Model(nodes, vocab_size, embedding_matrix,
                           inputLength, glove_vec_len)
    elif(model_string == "BiLSTM"):
        print("Executing Bidirectional LSTM")
        model = biLSTM_Model(nodes, vocab_size, embedding_matrix,
                             inputLength, glove_vec_len)
    elif(model_string == "SimpleGRU"):
        print("Executing Simple GRU")
        model = GRU_Model(nodes, vocab_size, embedding_matrix,
                          inputLength, glove_vec_len)
    elif(model_string == "BiGRU"):
        print("Executing Bidirectional GRU")
        model = biGRU_Model(nodes, vocab_size, embedding_matrix,
                            inputLength, glove_vec_len)
    ...
    mod_param_path = os.path.join(model_name, (model_type+model_name),
                                  category_name)
    category_name = category_name+".h5"
    if(save_model == "yes"):
        model.save(os.path.join("./main/models/", mod_param_path,
                                category_name))
        print("Model Saved at ", os.path.join("./main/models/",
                                              mod_param_path, category_name))
    elif(save_model == "no"):
...
def test_routine(model_name, model_type, category_name, tokenizer, model,
                 input_embedding, output_labels, seq_len,
    ...
    mod_param_path = os.path.join(model_name,
    ...
    if(save_predict_file == "yes"):
        ...
        predicted.to_csv(os.path.join("./main/output/", mod_param_path,
                                      category_name), index=False, header=True)
        print("Predicted File Saved")
    elif(save_predict_file == "no"):
        print("Predicted File Not Saved")

    return ([precision_recall_fscore_support(binary_pred, output_labels,
                                             average="binary"),
             accuracy_score(output_labels, binary_pred)])


def get_results(model_type, model_name, categories, cat_idx, iterations,
                epochs, nodes, vec_len, sequence_len, news, labels,
    ...
    for iteration in range(0, iterations):
        ...
        vars()[mod_string+"_model_up"], vars()[mod_string+"_model_acc"],\
            vars()[mod_string+"_model_loss"] = run_model(model_name,
                                                         ...
                                                         y_train[:, cat_idx],
                                                         nodes, epochs,
                                                         sequence_len, vec_len,
                                                         save_model=save_model)

        vars()[mod_string+"_out_up"], accuracy = test_routine(model_name,
                                                              model_type,
                                                              category_name,
                                                              tokenizer,
                                                              vars()
        ...
        vars()[mod_string + "_out"][iteration, :] = np.array(vars()
                                                             [mod_string +
                                                              "_out_up"])
        vars()[mod_string + "_acc"][iteration, :] = np.array(vars()
                                                             [mod_string +
                                                              "_model_acc"])
        vars()[mod_string + "_loss"][iteration, :] = np.array(vars()
                                                              [mod_string +
                                                               "_model_loss"])

    mod_param_path = os.path.join("./main/output/", model_name,
                                  (model_type + model_name), category_name)

    if save_results == "yes":

        with open(os.path.join(mod_param_path, "Scores.pkl"), "wb") as handle:
            pickle.dump(vars()[mod_string+"_out"], handle,
                        protocol=pickle.HIGHEST_PROTOCOL)

        with open(os.path.join(mod_param_path,
                               "Accuracy.pkl"), "wb") as handle:
            pickle.dump(vars()[mod_string+"_acc"], handle,
                        protocol=pickle.HIGHEST_PROTOCOL)

        with open(os.path.join(mod_param_path, "Loss.pkl"), "wb") as handle:
            pickle.dump(vars()[mod_string+"_loss"], handle,
                        protocol=pickle.HIGHEST_PROTOCOL)

        with open(os.path.join(mod_param_path,
                               "randomNumbers.pkl"), "wb") as handle:
            pickle.dump(random_seed_list, handle,
                        protocol=pickle.HIGHEST_PROTOCOL)
...
def get_data_ready(data, args):
    ...
    if args.remove_punct:
        data = data.apply(lambda x: "".join(ch
                                            for ch in x
    ...
    if args.remove_stopwords:
        ...
        for i, val in enumerate(data):
            ...
            filtered_sent = [w for w in temp_sent
...
if __name__ == "__main__":
    ...
    parser.add_argument("-seq_len", required=False, default=32,
                        help="Maximum sequence length of dataset", type=int)
    parser.add_argument("-itr", required=False, default=5,
                        help="Number of iterations", type=int)
    parser.add_argument("-epochs", required=False, default=5,
                        help="Number of epochs", type=int)
    parser.add_argument("-nodes", required=False, default=100,
                        help="Number of neurons in a layer", type=int)
    parser.add_argument("-vec_len", required=False, default=300,
                        help="Vector Length, based on what GloVe embedding\
                        is used", type=int)
    parser.add_argument("-remove_punct", action="store_true",
                        help="To remove punctuation", type=int)
    parser.add_argument("-remove_num", action="store_true", help="To remove \
                        numbers", type=int)
    parser.add_argument("-remove_stopwords", action="store_true",
                        help="To remove stopwords", type=int)
        ...
        for model_name in architecture_name:
            for cat_idx in range(0, 3):
                for i in range(0, 11):
                    ...
                    [accuracy, precision, recall, f1] = get_results(model_type,
                                                                    model_name,
                                                                    categories,
                                                                    cat_idx,
                                                                    iterations,
                                                                    ...
                                                                    "yes",
                                                                    "yes")
                    temp_ls = [model_type + model_name, i, categories[cat_idx],
                               accuracy, precision, recall, f1]
                    main_list.append(temp_ls)

    output_df = pd.DataFrame(main_list, columns=["Model", "Iteration",
                                                 "Category", "Accuracy",
                                                 "Precision", "Recall",
                                                 "F1_Score"])
