...
print (np.amin(num_tokens), np.mean(num_tokens), np.median(num_tokens),
        np.std(num_tokens), np.amax(num_tokens))
...
# Since it is multiclass dataste, it is wiser to look at the classification report
# and confusion matrix rather than using accuracy, precision on binary classification
# which can be achieved using "positive or not", "negative or not", etc.
