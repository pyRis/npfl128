...
statement_df = pd.read_csv('./data/stanford_data.csv', usecols=['sentences', 'labels'])
...
statement_df['num_words'] = statement_df['sentences'].apply(lambda x: len(x.split()))




pos_statements = ' '.join(statement_df.loc[statement_df['labels'] == 'pos']['sentences'].values)
neg_statements = ' '.join(statement_df.loc[statement_df['labels'] == 'neg']['sentences'].values)
neu_statements = ' '.join(statement_df.loc[statement_df['labels'] == 'neu']['sentences'].values)
...
df_val, df_test = train_test_split(df_test,
                                   test_size=0.5,
                                   random_state=42,
                                   stratify=df_test['labels'].values)

df_train_full = pd.concat([df_train, df_val])


def create_dataset(df, tokenizer, max_length):
    ds = StatementDataset(statements=df['sentences'].to_numpy(),
