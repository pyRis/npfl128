...
statement_df = pd.read_csv('merged.csv', usecols=['polarity', 'Title_new'])
...
statement_df['Title_new'] = statement_df['Title_new'].apply(clean_statements)
statement_df['num_char'] = statement_df['Title_new'].apply(len)
statement_df['num_words'] = statement_df['Title_new'].apply(lambda x: len(x.split()))




pos_statements = ' '.join(statement_df.loc[statement_df['polarity'] == 'buy']['Title_new'].values)
neg_statements = ' '.join(statement_df.loc[statement_df['polarity'] == 'sell']['Title_new'].values)
neutral_statements = ' '.join(statement_df.loc[statement_df['polarity'] == 'hold']['Title_new'].values)
...
le = LabelEncoder()
statement_df['polarity'] = le.fit_transform(statement_df['polarity'])
...
df_train, df_test = train_test_split(statement_df,
                                     ...
                                     stratify=statement_df['polarity'].values)

df_val, df_test = train_test_split(df_test,
                                   test_size=0.5,
                                   random_state=RANDOM_SEED,
                                   stratify=df_test['polarity'].values)

df_train_full = pd.concat([df_train, df_val])


def create_dataset(df, tokenizer, max_length):
    ds = StatementDataset(statements=df['Title_new'].to_numpy(),
...
def cv_ensemble_performance(preds, labels):
    ...
    print('')
    print(classification_report(y_true=labels, y_pred=preds, digits=3, target_names=target_strings))

def single_model_performance(preds, labels):
    print(confusion_matrix(y_true=labels, y_pred=preds))
    print('')
    print(classification_report(labels, preds, digits=4, target_names=target_strings))
...
def train_fold(epochs, model, device, train_dataloader,
    ...
    if single_model:
        ...
        single_model_performance(test_preds, df_test['polarity'].values)
...
distilbert_tokenizer = DistilBertTokenizer.from_pretrained(DISTILBERT_MODEL_NAME, return_dict=False)
...
class DistilBertForSequenceClassification(nn.Module):
    ...
    def __init__(self, pretrained_model_name, num_classes):
        ...
        config = AutoConfig.from_pretrained(pretrained_model_name, num_labels=num_classes, return_dict=False)
...
cv_ensemble_performance(distilbert_test_outputs, df_test['polarity'].values)
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           